...
class ChamadaRepository:
    ...
    def get_by_id(self, chamada_id: id) -> Optional[Chamada]:
        return self.db.query(Chamada).filter(Chamada.id == chamada_id).first()

    def get_by_turma(self, turma_id: id, skip: int = 0, limit: int = 100) -> List[Chamada]:
        return self.db.query(Chamada).filter(Chamada.turma_id == turma_id).offset(skip).limit(limit).all()

    def get_by_professor(self, professor_id: id, skip: int = 0, limit: int = 110) -> List[Chamada]:
